...
export interface MetadataConfig {
  ...
  foundingDate?: string;                                        // The founding date (year)
...
export interface CreditsConfig {
  ...
  freePool: {
    enable: boolean;                 // Whether to enable global free pool for anonymous users
    maxDailyCredits: number;         // Max credits per day for all anonymous users combined
    maxUserDailyCredits: number;     // Max credits per day for a single anonymous user
    maxCreditsPerModel: number;      // Max credits per model for anonymous users (limits model access)
  };
