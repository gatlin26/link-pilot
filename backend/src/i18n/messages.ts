...
// Arrays should be replaced entirely (not concatenated) when merging locale messages.
// deepmerge's default behavior concatenates arrays, which would cause translated
// array fields (e.g. inclusionTips.items) to include both English and translated items.
const overwriteArrayMerge = <T>(_dest: T[], source: T[]): T[] => source;
