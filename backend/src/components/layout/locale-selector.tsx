...
export default function LocaleSelector() {
  ...
  return (
    ...
    >
      <SelectTrigger className="h-8 w-auto min-w-0 cursor-pointer gap-1 px-2 pr-1.5">
        <SelectValue
          placeholder={<span>{websiteConfig.i18n.locales[locale].name}</span>}
        >
          {currentLocale && (
      ...
      <SelectContent className="min-w-0">
        {Object.entries(websiteConfig.i18n.locales).map(([cur, data]) => (
          <SelectItem
            key={cur}
            value={cur}
            className="cursor-pointer px-2 pr-7"
          >
            <span>{data.name}</span>
          </SelectItem>
        ))}
