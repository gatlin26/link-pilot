...
export const sites = pgTable('sites', {
	...
	sitesUserIdIdx: index('sites_user_id_idx').on(table.userId),
	sitesEnabledIdx: index('sites_enabled_idx').on(table.enabled),
}));

/**
 * Link Pilot V1.0 - 外链资源表
 */
export const backlinks = pgTable('backlinks', {
	...
	backlinksUserIdIdx: index('backlinks_user_id_idx').on(table.userId),
	backlinksDomainIdx: index('backlinks_domain_idx').on(table.domain),
	backlinksTypeIdx: index('backlinks_type_idx').on(table.type),
	backlinksQualityIdx: index('backlinks_quality_idx').on(table.quality),
	backlinksStatusIdx: index('backlinks_status_idx').on(table.status),
	backlinksFlaggedIdx: index('backlinks_flagged_idx').on(table.flagged),
}));

/**
 * Link Pilot V1.0 - 提交记录表
 */
export const submissions = pgTable('submissions', {
	id: text('id').primaryKey(),
	userId: text('user_id').notNull().references(() => user.id, { onDelete: 'cascade' }),
	siteId: text('site_id').notNull().references(() => sites.id, { onDelete: 'cascade' }),
	backlinkId: text('backlink_id').notNull().references(() => backlinks.id, { onDelete: 'cascade' }),
	submittedAt: timestamp('submitted_at').notNull(),
	submitMode: text('submit_mode').notNull(),
	commentUsed: text('comment_used').notNull(),
	...
	submissionsUserIdIdx: index('submissions_user_id_idx').on(table.userId),
	submissionsSiteIdIdx: index('submissions_site_id_idx').on(table.siteId),
	submissionsBacklinkIdIdx: index('submissions_backlink_id_idx').on(table.backlinkId),
	submissionsStatusIdx: index('submissions_status_idx').on(table.status),
