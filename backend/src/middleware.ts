 ...
 * Better Auth session cookie name: better-auth.session_token or {appName}.session_token
 */
function hasSessionCookie(req: NextRequest): boolean {
  const cookieHeader = req.headers.get('cookie') || '';
  // Better Auth session token cookie name
  const sessionCookieName = 'better-auth.session_token';
  return cookieHeader.includes(sessionCookieName);
}
...
export default async function middleware(req: NextRequest) {
  ...
  // Only check cookie existence, avoid API call to prevent loop and performance issues
