...
### SEO & Structured Data
- **Schema.org Implementation**: All structured data is managed in `src/lib/schema.ts`
- **Schema Renderer**: Use `MultipleSchemaRenderer` from `src/components/schema/schema-renderer.tsx` to render schemas
- **Available Schema Types**:
  - `generateOrganizationSchema()` - Organization information
  - `generateWebsiteSchema()` - Website search functionality
  - `generateToolProductSchema()` - Tool/product pages with complete merchant data
  - `generateProductSchema()` - Product schema with offers (for pages with ratings)
  - `generatePricingOfferSchema()` - Pricing page Offer schema (no aggregateRating/review)

  - `generateArticleSchema()` - Blog posts
  - `generateFAQPageSchema()` - FAQ pages
  - `generateBreadcrumbSchema()` - Navigation breadcrumbs
  - `generateAggregateRatingSchema()` - Rating information

#### Tool Pages Structured Data Requirements
For tool detail pages (`/tools/[slug]`), always include:
1. **Product Schema** with required fields:
   - `offers` with `hasMerchantReturnPolicy` and `shippingDetails`
   - `aggregateRating` (if tool has rating data)
   - `category` (tool categories)
   - `brand` information
2. **Breadcrumb Schema** for navigation
3. Use `generateToolProductSchema()` which includes all Google-required merchant fields

#### Validation
- Test structured data with [Google Rich Results Test](https://search.google.com/test/rich-results)
- Verify in Google Search Console for any structured data issues
- All product schemas must include either `offers`, `review`, or `aggregateRating`
...
- All structured data follows Schema.org standards and Google's merchant data requirements
